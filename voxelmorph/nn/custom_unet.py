...
class CustomUNet(models.BasicUNet):
    ...
    SpatialAttention3D(in_channels=getattr(block, 'out_channels', block_default_channels))
    for block, block_default_channels in zip(self.downsampling_conv_blocks, nb_features)
])
